...
public class TabIndicatorView extends LinearLayout {
    ...
    ItemDataCallBack itemDataCallBack;
    private Object[] callArray;
    ...
    public void addTabs(ITab[] iTabs) {
        this.tabs = iTabs;
        callArray = new String[iTabs.length];
    ...
    private void setTabListener(int i, View view) {
        view.setTag(i);
        final int j = i;
        tabs[i].setSelectAttrCallBack(new TabAttrCallBack() {
            @Override
            public void call(Object json) {
                ...
                if (itemDataCallBack != null) {
                    itemDataCallBack.call(callArray);
                }
    ...
    public void select(int index) {
        ...
        if (index > -1) {
            if (tabs[index].getConetntView() == null) {
                tabs[index].createContentView(getContext(),null);
            }
        }
        //再次选中自己
